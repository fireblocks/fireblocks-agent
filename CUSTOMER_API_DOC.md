...
This document provides a detailed overview of the Custom Server API, which facilitates communication between the Fireblocks Agent and the Customer Server in the Fireblocks Key Link enabled workspaces. The API allows for operations such as signing messages and retrieving the status of these messages.
...
| Field     | Type   | Required | Description                                  |
|-----------|--------|----------|----------------------------------------------|
| msgId     | string | Yes      | The ID used for tracking the message. When the same message is pulled from Fireblocks multiple times, the msgId will change. For persistency, refer to the requestId. Only the latest msgId associated with a requestId should be used in the response |
| deviceId  | string | Yes      | The ID of the device which is paired with the FB agent user id linked to the specific signing key in the request |
...
| Type   | Description                                                         |
|----------|---------------------------------------------------------------------|
| string | Possible values for the request: `EXTERNAL_KEY_PROOF_OF_OWNERSHIP_REQUEST`, `KEY_LINK_PROOF_OF_OWNERSHIP_REQUEST`, `KEY_LINK_TX_SIGN_REQUEST`. <br>**Note**: `EXTERNAL_KEY_PROOF_OF_OWNERSHIP_REQUEST` will be deprecated soon |

### ResponseType

| Type   | Description                                                         |
|----------|---------------------------------------------------------------------|
| string | Type of the response. Should be corresponding to the request. Possible values: `EXTERNAL_KEY_PROOF_OF_OWNERSHIP_RESPONSE`, `KEY_LINK_PROOF_OF_OWNERSHIP_RESPONSE`, `KEY_LINK_TX_SIGN_RESPONSE`. <br>**Note**: `EXTERNAL_KEY_PROOF_OF_OWNERSHIP_RESPONSE` will be deprecated soon |
  ...
  "messages": [
    {
        ...
        "transportMetadata": {
            "msgId": 478021000785,
            "deviceId": "f2300a10-676a-4de6-9d35-5abdce936383",
            "requestId": "3103feae24f9bf3513d95e8ca1344dbbfd11307e77570d86a7a0947ab80354f6",
            "type": "KEY_LINK_TX_SIGN_REQUEST"
        }
    }
]
...
```json
{
    "type": "KEY_LINK_TX_SIGN_RESPONSE",
    "status": "SIGNED",
    "request": "aa456e65-3aea-4bbb-9d90-f5cb8d833b1c",
    "response": {
        "signedMessages": [
            {
                "index": 0,
